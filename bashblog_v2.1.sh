...
# FTP - configure the host, user and directories - CONFIGURE!
FTP_HOST="hostname"
FTP_USER="username"
FTP_DIR="/hostdirectory"
FTP_DIR_POSTS="/hostdirectory/$DIR_POSTS/"
FTP_DIR_TAGS="/hostdirectory/$DIR_TAGS/"
...
CHECK_NCFTP(){
command -v ncftp &> /dev/null
if [ $? -eq 1 ]; then
echo >&2 "CHECK NCFTP: ncftp is not installed."
exit 1
else
echo >&2 "CHECK NCFTP: OK"
fi
}
...
FTP_NCFTP(){
PASSWORD
ncftp -u "${FTP_USER}" -p "${password}" "${FTP_HOST}"
unset password
}

# FTP - sends the main files and directories 
FTP_SEND_ALL_FILES(){
PASSWORD
ncftpput -R -v -u "${FTP_USER}" -p "${password}" "${FTP_HOST}" "${FTP_DIR}" "${INDEXHTML}" "${DIR_POSTS}" "${DIR_CSS}" "${DIR_TAGS}" "${DIR_IMG}"
unset password
}

# FTP - send specific files to the server 
FTP_SEND_FILES(){
read -p "Number of files to be uploaded to the server: " num_files
PASSWORD
for i in $(seq 1 ${num_files})
do
read -ep "FILE NAME[$i]: " file_name
ncftpput -R -v -u "${FTP_USER}" -p "${password}" "${FTP_HOST}" "${FTP_DIR}" "${file_name}"
done
unset password
}
...
case $1 in
             ...
             "-ftp-ncftp" | "-fn")	CHECK_FILES ; CHECK_NCFTP ; FTP_NCFTP					;	;;
             "-ftp-send-all-files" | "-fsaf")	CHECK_FILES ; CHECK_NCFTP ; FTP_SEND_ALL_FILES			;	;;
